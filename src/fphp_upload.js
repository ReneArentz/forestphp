$(function() {
				...
				xhr.upload.addEventListener('progress', function(p_e_event) {
					if (p_e_event.lengthComputable) {
						...
						if (i_percentComplete === 100) {
							...
							$(o_JSON_upload['s_uploadStatusId'] + p_s_randomId).removeClass('label-default').addClass('label-success');
						}
					}
				}, false);
	...
	function fphp_AddFileToList(p_s_randomId, p_s_fileName, p_s_fileSize) {
		...
		var s_upload_status = '<span id="' + o_JSON_upload['s_uploadStatusIdValue'] + p_s_randomId + '" class="label label-default">0%</span>';
		var s_upload_delete = '<span id="' + o_JSON_upload['s_uploadDeleteIdValue'] + p_s_randomId + '" class="glyphicon glyphicon-remove text-danger" style="cursor: pointer;"></span>';
